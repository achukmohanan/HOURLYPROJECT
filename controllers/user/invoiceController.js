...
doc.fontSize(11)
   ...
   .text(`Email: ${order.userId.email}`, 70, currentY + 40,{ width: 260 });
   doc.fontSize(11)
   .font('Helvetica')
   .fillColor(colors.lightText)
   .text(`Address: ${order.address.addressType},${order.address.name},${order.address.city},${order.address.landMark},${order.address.state}.${order.address.pincode  }`, 70, currentY + 60 ,{ width: 260 });
...
doc.rect(50, tableTop, tableWidth, rowHeight)
   .fill(colors.primary); // fill background color
...
doc.text('Product Name', 100, tableTop + 8, { width: 200, align: 'left' });
...
order.orderedItems.forEach((item, index) => {
    ...
    doc.fillColor(colors.text)
       ...
       .text(item.quantity.toString(), 320, currentY + 12, { align: 'center' })
       .text(`₹${item.product.salePrice.toLocaleString()}`, 380, currentY + 12, { align: 'right', width: 70 })
       .text(`₹${itemTotal.toLocaleString()}`, 470, currentY + 12, { align: 'right', width: 70 });
...
addBox(totalsX, currentY, totalsWidth, 100, colors.secondary, colors.border);
...
doc.fillColor(colors.text)
   ...
   .text(`₹${subtotal.toLocaleString()}`, totalsX + 150, currentY + 20, { align: 'right', width: 70 });
...
doc.fillColor(colors.primary)
   ...
   .text(`₹${order.totalPrice.toLocaleString()}`, totalsX + 150, currentY + 70, { align: 'right', width: 70 });
...
doc.fontSize(11)
   .font('Helvetica')
   .text('We appreciate your trust in our services. For any queries, contact us at hourlywatches@gmail.com', 
         50, currentY + 50, { 
             align: 'center', 
             width: 495 
         });
