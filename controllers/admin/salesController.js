...
const filterSales = async(req,res) => {
    try {
       ...
       //chose filter 
        ...
        const result = report[0]|| { totalSales: 0, totalOrders: 0, totalQuantity: 0 };
  ...
  // Title
  doc.fontSize(20).fillColor("#2c3e50").text("Sales Report", { align: "center" });
  doc.moveDown(1);
  ...
  const headers = ["User", "Email", "Product", "Qty", "Price", "Payment", "Coupon", "Total"];
  const tableTop = 100;
  const rowHeight = 25;
  ...
  orders.forEach((order, orderIndex) => {
    order.orderedItems.forEach((item, itemIndex) => {
      ...
      if ((orderIndex + itemIndex) % 2 === 0) {
        doc.rect(doc.page.margins.left, y, colWidths.reduce((a, b) => a + b, 0), rowHeight)
          .fill("#ecf0f1")
          .stroke();
      }
  ...
  doc.font("Helvetica-Bold")
    ...
    .text(`Total Quantity: ${totalQuantity}`, doc.page.margins.left + 200, y)
